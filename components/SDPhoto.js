...
import { COSTS_UPSCALER, ERROR_CODE_NO_CREDITS } from "@/utils/consts";
...
export function SDPhoto({src, status, id, isUpscaled ,index, moreLikeThis, resetMoreLikeThis, reduceUserCredits}) {
    ...
    */
    async function upscaleDone(resp) {
      ...
      if(objResp.status > 201) {
        setError("Error while saving the upscaled image: ", objResp.error || objResp.message)
      }
      setImgSrc(objResp.url)
      setIsPhotoUpscaled(true)
      reduceUserCredits(COSTS_UPSCALER)
   ...
   async function upscalePhoto() {
    ...
    if(upscaleResp.error_code == ERROR_CODE_NO_CREDITS) {
      setUpscaling(false)
      return setError("You don't have enough tokens for this operation")
    }
    ...
    while (
      ...
      upscaleResp.status !== "failed"
        ...
        if (response.status !== 200 || upscaleResp.error) {
          setError(upscaleResp.error || upscaleResp.message);
          setUpscaling(false)
          return;
        }
    ...
    return (
        <>
          ...
          <div className={ "position-relative img-wrapper " + (isPhotoUpscaled? 'upscaled-photo' : '') }>
            ...
            */}
            {isPhotoUpscaled && <div className="pill upscaled-pill">Upscaled</div>}
          
            <Button variant="primary" className="position-absolute top-0 start-0 p-2 overlay-button" style={{ opacity: (upscaling? 1 : 0) }} onClick={upscalePhoto} disabled={upscaling}  
                    title={"Upscale the image. Cost: " + COSTS_UPSCALER + " tokens"}
                      >
